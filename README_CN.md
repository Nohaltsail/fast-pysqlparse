...
- **对比sqlparse**: 快 **75x**
- **对比sqlglot**: 快 **25x**
- **吞吐量**: 8000 PPS (Parses Per Second)
- **大文件处理**: 1000万字符SQL仅需1.4秒 (7,500,000 CPS)
...
parsed = Parsed(sql)
# 提取源表和目标表
for index, parsed in enumerate(parsed.parsedforest):
...
**属性**:
- `Parsed`: AbstractParsed - 解析后的结构对象
- `type`: str - 语句类型
- 
**方法**:
- `tokens()`: List[Token] - 获取Token列表
